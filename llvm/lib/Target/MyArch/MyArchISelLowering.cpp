...
static const MCPhysReg ArgGPRs[] = {MyArch::R9, MyArch::R10, MyArch::R11, MyArch::R12};
...
MyArchTargetLowering::MyArchTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(MyArch::R1);
...
SDValue MyArchTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, MyArch::R1, PtrVT);
      SDValue Address =
